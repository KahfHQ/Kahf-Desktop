...
describe('sgnlHref', () => {
  ...
  describe('isSignalHttpsLink', () => {
    ...
    it('returns false if the protocol is not "https:"', () => {
      assert.isFalse(isSignalHttpsLink('kahf://signal.art', explodingLogger));
      assert.isFalse(
        isSignalHttpsLink(
          'kahf://signal.art/addstickers/?pack_id=abc',
          explodingLogger
        )
      );
      assert.isFalse(
  ...
  describe('parseSgnlHref', () => {
    ...
    it("parses a command's arguments", () => {
      assert.deepEqual(
        parseSgnlHref(
          'kahf://Foo?bar=baz&qux=Quux&num=123&empty=&encoded=hello%20world',
          explodingLogger
        ),
        {
          command: 'Foo',
    ...
    it('treats the port as part of the command', () => {
      assert.propertyVal(
        parseSgnlHref('kahf://foo:1234', explodingLogger),
        'command',
        'foo:1234'
      );
    });
    ...
    it('ignores duplicate query parameters', () => {
      assert.deepPropertyVal(
        parseSgnlHref('kahf://x?foo=bar&foo=totally-ignored', explodingLogger),
        'args',
        new Map([['foo', 'bar']])
      );
    });
    ...
    it("doesn't do anything fancy with arrays or objects in the query string", () => {
      ...
      assert.deepPropertyVal(
        parseSgnlHref('kahf://x?foo[]=bar&foo[]=baz', explodingLogger),
        'args',
        new Map([['foo[]', 'bar']])
      );
      assert.deepPropertyVal(
        parseSgnlHref('kahf://x?foo[bar][baz]=foobarbaz', explodingLogger),
        'args',
